...
def main():
    ...
    parser.add_argument(
        "--stats", help="Dump stats of number of particles per event to file"
    )
    parser.add_argument(
        "--eventnumber",
        action="store_true",
    ...
    if len(events) > 1 and not args.eventnumber:
        print(
            "Warning, multiple events will be written to the same file without eventnumbers"
        )
    ...
    with open(args.output, "w") as outf:
        for ievent, event in enumerate(events):
            for particle in event:
                ...
                print(
                    f"{particle.px} {particle.py} {particle.pz} {particle.E}", file=outf
                )
            if args.stats:
